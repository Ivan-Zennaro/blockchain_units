...
public class UserMain {
	...
	//fonti di input dalle quali prendere le info necessarie
	...
	public static void main (String argv[]) {
		...
		try{
			...
			if(argv.length != 0)		//controllo, se c'� un paramtreo in args allora uso quello come ip
				IPNODE = argv[0];		//mi fido che sia un indirizzo ip, non controllo, altrimenti termina il programma
			else{						//altrimenti prendo l'ip in automatico
				System.out.println("Inserisci indirizzo IP del nodo a cui ti vuoi connettere: ");
			...
			while(true) {			
				...
				if (isConnect) {  //la variabile connesso la gestisce il thread

					//invio la mia chiave pubblica 
					outputStreamObj.writeObject(publicKey);
					outputStreamObj.flush();

					while (isConnect) { //fino a quando rimane connesso esegue questo ciclo
					 ...
					 if(inp == 1) {
						 ...
						 output.println("?-WALLETS"); // richiedo gli wallets
						 Thread.sleep(350);		  // attendo il loro arrivo
						 if(walletsArrive) {
							 walletsArrive = false;
						 ...
						 else {System.out.println("Error... No Wallet arrived");}
					 }//inp==1
		...
		catch(Exception e){System.out.println("Impossible to close the connection");}}//finally
	}//main
	...
	public static float getBalance() {
		output.println("?-BALANCE");   //richiedo il saldo
	...
	public static Transaction generateTransaction(PublicKey _recipient,float value ) {
		...
		//il vettore delle transazioni di input sar� vuoto
